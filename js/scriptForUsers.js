...
function gamePlayers() {
    var countGame = $(".gamePlayer").data("count_game");
    for (var i = 0; i < countGame; i++) {
        var count = $("#g" + i).data("count");
        if (count == 0)
            $("#g" + i).css('background', '#9f9');
        else if (count == 1)
            $("#g" + i).css('background', '#f99');
        else if (count == 2)
            $("#g" + i).css('background', '#99f');
        //alert(count);
    }
}
...
{
    /*$(document.body).on("click", ".list-view", function (event)
    {
